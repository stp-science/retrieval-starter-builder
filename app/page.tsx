...
export default function Home() {
  ...
  const courseFileLabel = courseLabel.replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "");
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      if (activity === "walkabout-bingo") {
        ...
        pdf.save(`${courseFileLabel}-${activityName}.pdf`);
        return;
      }
  ...
  async function downloadWord() {
    ...
    try {
      ...
      const content: (InstanceType<typeof Paragraph> | InstanceType<typeof Table>)[] = [
        ...
        para(`${courseLabel}  •  ${selectedTopics.map((topic) => topic.name).join("  •  ")}`, { center: true, color: "5D6C7B", size: 18, after: 150 }),
        new Table({
      ...
      const activityName = titleForActivity(activity).replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "");
      link.href = url;
      link.download = `${courseFileLabel}-${activityName}.docx`;
  ...
  return (
    <main>
      <header className="topbar">
        ...
        <span className="beta-pill">{topics.reduce((sum, topic) => sum + topic.questions.length, 0)}-question bank</span>
      </header>
